...
func NewService(repo user_repository.UserRepository, logger log.Logger) Service {
	return &service{
		repo:   repo,
		logger: log.With(logger, "service", "add_user"),
	}
}


func (s *service) AddUser(ctx context.Context, req Request) (Response, error) {
	...
	_, err = s.repo.GetUserByUsernameOrEmail(req.Username, req.Email)
	if err == nil {
